...
OHLCV_DAILY_DIR = r"data/ohlcv_daily"
...
# Small-RAM Lightsail-safe defaults
LIGHTSAIL_SMALL_RAM_MODE = True
...
VALID_DAYS_MAX = 90
VALID_DAYS_MIN = 20
MAX_TRAIN_SAMPLES_PER_BUCKET = 20_000
MAX_VAL_SAMPLES_PER_BUCKET   = 8_000
...
ROCKET_CFG = dict(kernels={7: 128, 15: 128, 21: 256}, max_dilation=4)
ROCKET_RANDOM_SEED = 4242
ROCKET_BATCH_SIZE = 2000
...
PRED_BATCH = 20_000
ROCKET_MEMMAP_DIR = os.path.join(OUT_DIR, "features")
...
RR_POLICY_MAX_ROWS_PER_PAIR = 12_000
...
def _series_cache_key(csv_path: str, symbols_needed: List[str]) -> str:
    base_name = os.path.basename(csv_path).replace(" ", "_").replace("(", "").replace(")", "")
    st = os.stat(csv_path)
    sym_blob = "\n".join(sorted({str(s).upper().strip() for s in symbols_needed}))
    sym_hash = hashlib.sha1(sym_blob.encode("utf-8")).hexdigest()[:12]
    return f"{base_name}_gap{SUSP_GAP_DAYS}_m{int(st.st_mtime_ns)}_sz{int(st.st_size)}_sy{sym_hash}"
...
def load_ohlcv_subset_build_series(csv_path: str, symbols_needed: List[str], atr_period_default: int):
    ...
    Cache key includes OHLCV file fingerprint + exact symbol-set hash to avoid stale/colliding caches.
    """
    symbols_needed = [s.upper() for s in symbols_needed]
    symset = set(symbols_needed)

    cache_key = _series_cache_key(csv_path, symbols_needed)
    cache_series = os.path.join(OUT_DIR, "caches", f"series_cache_{cache_key}.pkl")
    cache_seg = os.path.join(OUT_DIR, "caches", f"seg_map_{cache_key}.pkl")
...
def _rocket_memmap_path(prefix: str) -> str:
    os.makedirs(ROCKET_MEMMAP_DIR, exist_ok=True)
    fd, path = tempfile.mkstemp(prefix=f"{prefix}_", suffix=".mmap", dir=ROCKET_MEMMAP_DIR)
    os.close(fd)
    return path

def compute_rocket_features_memmap_for_df(df: pd.DataFrame, series_map: Dict[str, SymSeries], rocket_cfg: dict, seed: int,
                                          prefix: str) -> Tuple[str, int, np.ndarray]:
    """
    Computes ROCKET features in chunks and stores them in an on-disk memmap.

    Returns
    -------
    mmap_path : str
        Path to memmap file with shape (n, rocket_dim), dtype=FEATURE_DTYPE.
    d : int
        ROCKET feature dimension.
    ok_all : np.ndarray
        (n,) uint8, 1 when all ROCKET windows were valid for that row.
    """
    cfg = normalize_rocket_cfg(rocket_cfg)
    n = len(df)
    d = rocket_feature_dim(cfg)
    mmap_path = _rocket_memmap_path(prefix)
    F = np.memmap(mmap_path, mode="w+", dtype=FEATURE_DTYPE, shape=(n, d))
    ...
    for start in range(0, n, ROCKET_BATCH_SIZE):
        ...
        for L in ROCKET_WINDOWS:
            ...
            Fw = featurizers[L].transform_batch(Xw).astype(FEATURE_DTYPE, copy=False)
            a, b = offsets[L]
            F[start:end, a:b] = Fw
        ...
        if ((start // ROCKET_BATCH_SIZE) + 1) % 10 == 0 or end == n:
            F.flush()

    F.flush()
    del F
    return mmap_path, d, ok_all

def materialize_feature_rows(mmap_path: str, n_rows: int, d: int, row_idx: np.ndarray,
                             F_static: Optional[np.ndarray] = None,
                             out_dtype: np.dtype = TRAIN_DTYPE) -> np.ndarray:
    row_idx = np.asarray(row_idx, dtype=np.int64)
    F = np.memmap(mmap_path, mode="r", dtype=FEATURE_DTYPE, shape=(n_rows, d))
    X = np.asarray(F[row_idx], dtype=out_dtype)
    del F
    if F_static is not None and F_static.shape[1] > 0:
        Xs = np.asarray(F_static[row_idx], dtype=out_dtype)
        X = np.hstack([X, Xs]).astype(out_dtype, copy=False)
    return X

def materialize_feature_all(mmap_path: str, n_rows: int, d: int,
                            F_static: Optional[np.ndarray] = None,
                            out_dtype: np.dtype = TRAIN_DTYPE) -> np.ndarray:
    F = np.memmap(mmap_path, mode="r", dtype=FEATURE_DTYPE, shape=(n_rows, d))
    X = np.asarray(F, dtype=out_dtype)
    del F
    if F_static is not None and F_static.shape[1] > 0:
        Xs = np.asarray(F_static, dtype=out_dtype)
        X = np.hstack([X, Xs]).astype(out_dtype, copy=False)
    return X
...
# =========================================================
# Paper state (pending orders + open positions + ledger)
# =========================================================
@dataclass
class OpenPos:
    pos_id: int
...
class PendingOrder:
    pos_id: int
    entry_date: str           # intended fill date YYYY-MM-DD
    symbol: str
    side: str
    regime: str
    side_mode: str
    ...
    gate_mult: float
    score: float
    ref_date: str             # date used for sizing inputs
    ref_atr_value: float      # ATR known on ref_date
    expr_score: float
    expr_score_adj: float
    rr_pred: float
    model_run: str
...
def _daily_capacity(equity: float, open_positions: List[OpenPos], pending_orders: Optional[List[PendingOrder]] = None) -> Tuple[int, float]:
    pending_orders = pending_orders or []
    slots = max(0, int(MAX_OPEN_POSITIONS - len(open_positions) - len(pending_orders)))
    open_risk = float(sum(p.risk_amt for p in open_positions)) + float(sum(p.risk_amt for p in pending_orders))
...
def load_pending_orders() -> List[PendingOrder]:
    fp = os.path.join(OUT_DIR, "state", "pending_orders.csv")
    if not os.path.exists(fp) or os.path.getsize(fp) == 0:
        return []
    try:
        df = pd.read_csv(fp)
    ...
    out = []
    for r in df.to_dict("records"):
        out.append(PendingOrder(**r))
    return out

def save_pending_orders(orders: List[PendingOrder]):
    fp = os.path.join(OUT_DIR, "state", "pending_orders.csv")
    if not orders:
        pd.DataFrame(columns=list(PendingOrder.__annotations__.keys())).to_csv(fp, index=False)
        return
    pd.DataFrame([asdict(p) for p in orders]).to_csv(fp, index=False)
...
def ensure_ledger_file():
    ...
    if os.path.exists(fp):
        return
    cols = [
        "pos_id","symbol","side","regime","side_mode","stop_r","tgt_r",
        "entry_date","exit_date","exit_reason","entry_price","exit_price",
        "rmult_used","risk_amt","gate_mult","score","pnl","equity_before",
        "equity_after","marked_to","model_run"
    ]
...
def build_training_window(run_date: pd.Timestamp) -> Tuple[pd.Timestamp, pd.Timestamp]:
    cutoff = pd.Timestamp(run_date).normalize()
...
def train_bucket_models_daily(train_df: pd.DataFrame, series_map: Dict[str, SymSeries], run_tag: str) -> Tuple[Dict[Tuple[str,str], BucketModels], pd.DataFrame]:
    ...
    Memory-safe rewrite:
    - ROCKET features are computed chunk-wise into a memmap on disk.
    - Per-bucket train/val arrays are materialized only for the selected rows.
    """
    if train_df.empty:
        raise RuntimeError("train_df empty")

    rocket_mmap_path, rocket_dim, ok_all = compute_rocket_features_memmap_for_df(
        train_df, series_map, ROCKET_CFG, ROCKET_RANDOM_SEED, prefix=f"train_{run_tag}"
    )
    ...
    try:
        for regime in ["bull","bear"]:
            for side in ["long","short"]:
                ...
                X_tr = materialize_feature_rows(rocket_mmap_path, len(df), rocket_dim, idx_tr, F_static=F_static)
                X_va = materialize_feature_rows(rocket_mmap_path, len(df), rocket_dim, idx_va, F_static=F_static)
        ...
        try:
            if os.path.exists(rocket_mmap_path):
                os.remove(rocket_mmap_path)
        except Exception:
            pass
...
def train_rr_policy_daily(train_df: pd.DataFrame, series_map: Dict[str, SymSeries], run_tag: str) -> Tuple[Dict[Tuple[str,str,float,float], RRPolicyPack], Dict[Tuple[float,float], RRPolicyPack], pd.DataFrame]:
    ...
    rocket_mmap_path, rocket_dim, ok_all = compute_rocket_features_memmap_for_df(
        train_df, series_map, ROCKET_CFG, ROCKET_RANDOM_SEED, prefix=f"rr_{run_tag}"
    )
    ...
    try:
        if RR_POLICY_BUCKET_AWARE:
            for (regime, side, stop_r, tgt_r), g in df.groupby(["regime","side","stop_r","tgt_r"], sort=False):
                ...
                X = materialize_feature_rows(rocket_mmap_path, len(train_df), rocket_dim, idx, F_static=F_static)
...
def make_rr_candidates(div_df: pd.DataFrame, run_date: pd.Timestamp) -> pd.DataFrame:
    """
    Nightly run logic:
    - divergences_latest.csv is built from the latest OHLCV day (today close)
    - fresh signals confirmed on run_date typically get EntryDate = next business day
    - therefore, a nightly paper-trader run should build candidates for the next
      tradable business day, not for run_date itself

    Expands RR combos: (stop_r, tgt_r).
    """
    decision_day = pd.Timestamp(run_date).normalize()
    next_trade_day = (decision_day + BDay(1)).normalize()
    d0 = div_df[div_df["entry_date"] == next_trade_day].copy()
    if d0.empty:
        return d0
...
def get_ohlc_on_or_before(series: SymSeries, day: pd.Timestamp) -> Optional[Tuple[pd.Timestamp,float,float,float,float,float]]:
    if series is None or series.ohlc is None or len(series.dates64) == 0:
        return None
    day64 = np.datetime64(pd.Timestamp(day).normalize().date())
    i = np.searchsorted(series.dates64, day64, side="right") - 1
    if i < 0:
        return None
    o, h, l, c = series.ohlc[i]
    atr = series.atr[i] if series.atr is not None else np.nan
    actual_day = pd.Timestamp(str(series.dates64[i])).normalize()
    return actual_day, float(o), float(h), float(l), float(c), float(atr)

def get_daily_file_open(symbol: str, day: pd.Timestamp) -> Optional[Tuple[float, float, float, float]]:
    day = pd.Timestamp(day).normalize()
    fp = os.path.join(OHLCV_DAILY_DIR, f"ohlcv_{day.date().isoformat()}.csv.gz")
    if not os.path.exists(fp):
        return None
    try:
        df = pd.read_csv(fp, usecols=["date", "symbol", "open", "high", "low", "close"])
    except Exception:
        return None
    if df.empty:
        return None
    ...
    row = df[df["symbol"] == str(symbol).upper().strip()]
    if row.empty:
        return None
    r = row.iloc[0]
...
def simulate_exit_from_entry(
    ...
    true_time_day = entry_day + pd.Timedelta(days=int(max_hold_days))
    last_day = min(cutoff_day, true_time_day)
    ...
    # 3) TIME exit only after the true max-hold horizon is actually reached
    if cutoff_day >= true_time_day:
        px = get_ohlc_at(series, true_time_day)
        if px is None:
            return None, None, None
        return true_time_day, "TIME", float(px[3])
...
def main():
    ...
    symbols_needed = sorted(set(train_trades["symbol"].unique().tolist()) | set(div["symbol"].unique().tolist()) | set([p.symbol for p in open_positions]) | set([p.symbol for p in pending_orders]))
    ...
    # ---- Activate pending orders that should fill on or before run_date ----
    activated_rows = []
    still_pending = []
    fill_skips = Counter()
    for po in pending_orders:
        entry_day = pd.Timestamp(po.entry_date).normalize()
        if entry_day > run_date:
            still_pending.append(po)
            fill_skips["future_entry_day"] += 1
            continue
        s = series_map.get(po.symbol)
        px = get_ohlc_at(s, entry_day) if s is not None else None
        if px is None:
            daily_px = get_daily_file_open(po.symbol, entry_day) if 'get_daily_file_open' in globals() else None
            if daily_px is not None:
                o, h, l, c = daily_px
                atr_today = np.nan
                fill_skips["used_daily_file_fallback"] += 1
            else:
                still_pending.append(po)
                fill_skips["missing_entry_day_ohlc"] += 1
                continue
        else:
            o, h, l, c, atr_today = px
        atr_ref = float(po.ref_atr_value) if np.isfinite(po.ref_atr_value) and po.ref_atr_value > 0 else np.nan
        if not np.isfinite(atr_ref) or atr_ref <= 0:
            prev = get_ohlc_on_or_before(s, entry_day - pd.Timedelta(days=1)) if s is not None else None
            if prev is None:
                prev = get_ohlc_on_or_before(s, entry_day) if s is not None else None
            if prev is None:
                still_pending.append(po)
                fill_skips["missing_atr_reference"] += 1
                continue
            _, _, _, _, _, atr_prev = prev
            atr_ref = float(atr_prev) if np.isfinite(atr_prev) and atr_prev > 0 else np.nan
        if not np.isfinite(atr_ref) or atr_ref <= 0:
            still_pending.append(po)
            fill_skips["invalid_atr_reference"] += 1
            continue
        entry_price = float(o)
        stop_dist = float(po.stop_r) * atr_ref
        tgt_dist  = float(po.tgt_r) * atr_ref
        if str(po.side).lower() == "long":
        ...
        open_positions.append(OpenPos(
            pos_id=int(po.pos_id),
            entry_date=str(entry_day.date()),
            symbol=str(po.symbol),
            side=str(po.side),
            regime=str(po.regime),
            side_mode=str(po.side_mode),
            stop_r=float(po.stop_r),
            tgt_r=float(po.tgt_r),
            entry_price=float(entry_price),
            stop_price=float(stop_price),
            target_price=float(target_price),
            risk_amt=float(po.risk_amt),
            gate_mult=float(po.gate_mult),
            score=float(po.score),
            last_mark_date="",
        ))
        activated_rows.append(dict(
            pos_id=int(po.pos_id), symbol=str(po.symbol), side=str(po.side), entry_date=str(entry_day.date()),
            filled_entry_price=float(entry_price), stop_price=float(stop_price), target_price=float(target_price),
            stop_r=float(po.stop_r), tgt_r=float(po.tgt_r), risk_amt=float(po.risk_amt), gate_mult=float(po.gate_mult),
            expr_score=float(po.expr_score), expr_score_adj=float(po.expr_score_adj), rr_pred=float(po.rr_pred) if np.isfinite(po.rr_pred) else np.nan,
            model_run=str(run_date.date()), activation_day=str(run_date.date())
        ))
    pending_orders = still_pending
    log.info(f"[FILL] activated={len(activated_rows):,} pending_left={len(pending_orders):,} skip_reasons={dict(fill_skips)}")

    # ---- Close / mark actual open positions up to cutoff (= run_date close for nightly run) ----
    ...
    for p in open_positions:
        ...
        last_mark = pd.Timestamp(p.last_mark_date).normalize() if p.last_mark_date else (entry_day - pd.Timedelta(days=1))
    ...
    if cand.empty:
        log.info("[CAND] no candidates for next session; writing empty outputs.")
    ...
    # reference price/ATR from latest available bar on or before intended entry day
    ref_dates = []
    entry_prices = []
    stop_prices = []
    target_prices = []
    ...
    for r in cand.itertuples(index=False):
        ...
        px = get_ohlc_on_or_before(s, r.entry_date) if s is not None else None
        if px is None:
            ref_dates.append(pd.NaT); entry_prices.append(np.nan); stop_prices.append(np.nan); target_prices.append(np.nan); atr_vals.append(np.nan); ok_px.append(0)
            continue
        ref_day, o, h, l, c, atr = px
        atr_use = atr if np.isfinite(atr) and atr > 0 else np.nan
        if not np.isfinite(atr_use) or atr_use <= 0:
            ref_dates.append(pd.NaT); entry_prices.append(np.nan); stop_prices.append(np.nan); target_prices.append(np.nan); atr_vals.append(np.nan); ok_px.append(0)
            continue
        # proxy entry for scoring/ranking only; actual paper fill will happen later at entry-day open
        entry = float(c)
    ...
    cand["ref_date"] = pd.to_datetime(ref_dates, errors="coerce").normalize()
    ...
    cand_mmap_path, cand_rocket_dim, ok_all_c = compute_rocket_features_memmap_for_df(
        cand, series_map, ROCKET_CFG, ROCKET_RANDOM_SEED, prefix=f"cand_{run_tag}"
    )
    try:
        F_static_c, static_cols_c = build_static_features(cand)
        Xc = materialize_feature_all(cand_mmap_path, len(cand), cand_rocket_dim, F_static=F_static_c)
    finally:
        try:
            if os.path.exists(cand_mmap_path):
                os.remove(cand_mmap_path)
        except Exception:
            pass
    ...
    cap, open_risk_pct = _daily_capacity(equity, open_positions, pending_orders)
    if cap <= 0:
        log.info(f"[SELECT] cap=0 open_positions={len(open_positions)} pending_orders={len(pending_orders)} open_risk_pct={open_risk_pct:.3f}")
    ...
    # create pending orders for next session; actual fills happen when that entry_date bar becomes available
    orders = []
    for r in cand_sel.itertuples(index=False):
        ...
        atr_ref = float(r.atr_value) if np.isfinite(r.atr_value) and r.atr_value > 0 else np.nan
        if not np.isfinite(atr_ref) or atr_ref <= 0:
            continue
        ...
        pending_orders.append(PendingOrder(
            ...
            ref_date=str(pd.Timestamp(r.ref_date).normalize().date()) if pd.notna(r.ref_date) else str(run_date.date()),
            ref_atr_value=float(atr_ref),
        ...
        stop_dist = float(r.stop_r) * float(atr_ref)
        tgt_dist = float(r.tgt_r) * float(atr_ref)
        px_proxy = float(r.entry_price)
        if r.side == "long":
            stop_proxy = px_proxy - stop_dist
            tgt_proxy = px_proxy + tgt_dist
        else:
            stop_proxy = px_proxy + stop_dist
            tgt_proxy = px_proxy - tgt_dist
        q = qty_from_risk(risk_amt, px_proxy, stop_proxy, r.side)
        ...
        orders.append(dict(
            ...
            entry_date=str(pd.Timestamp(r.entry_date).normalize().date()),
            ref_date=str(pd.Timestamp(r.ref_date).normalize().date()) if pd.notna(r.ref_date) else str(run_date.date()),
            entry_price_proxy=float(px_proxy),
            qty=int(q),
            stop_price_proxy=float(stop_proxy),
            target_price_proxy=float(tgt_proxy),
            stop_r=float(r.stop_r),
            tgt_r=float(r.tgt_r),
            atr_value_ref=float(atr_ref),
            risk_amt=float(risk_amt),
            gate_mult=float(gm),
            expr_score=float(r.expr_score),
            expr_score_adj=float(r.expr_score_adj),
    ...
    log.info(f"[DONE] orders={len(orders):,} pending_orders={len(pending_orders):,} open_positions={len(open_positions):,} equity={equity:,.2f} elapsed={time.time()-t0:.1f}s")
